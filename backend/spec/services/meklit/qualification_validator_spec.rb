...
RSpec.describe Meklit::QualificationValidator, type: :service do
  ...
  describe '#call' do
    ...
    context 'when student status is invalid' do
      it 'adds error for invalid status' do
        student.update!(status: 'registered')
        validator = described_class.new(student)
        expect(validator.call).to be false
        expect(validator.errors).to include(/Student status 'registered' is not valid for export/)
      end
    end
